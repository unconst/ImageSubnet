...
async def main():
    ...
    for prompt in prompts:
        ...
        # take best image
        best_image = best_images[rewards.argmax()]
        if best_image is None:
            bt.logging.trace("Best image is None, skipping block")
            return
        ...
        # create sub folder called "imgoutputs-i2i" put all images in there
        if not os.path.exists('imgoutputs-i2i'):
            os.makedirs('imgoutputs-i2i')
        ...
        # save tiled image
        tiled_image.save(f'imgoutputs-i2i/{curr_block}/tiled.png')

        # do the same for inverted rewards
        zipped = list(zip(responses, _uids))
        ...
        # save tiled image
        tiled_image.save(f'imgoutputs-i2i/{curr_block}/tiled-inverted-score.png')
        ...
        # save tiled image
        tiled_image.save(f'imgoutputs-i2i/{curr_block}/tiled-score.png')

        # save a file with all uids and scores
        with open(f'imgoutputs-i2i/{curr_block}/scores.txt', 'w') as f:
            for i, uid in enumerate(_uids):
                coldkey = coldkeys[list(uids).index(uid)]
                f.write(f'{uid},{coldkey},{trim(old_rewards[i])},{trim(rewards[i])},{trim(inverted_rewards[i])}\n')
        
        # write prompt to file
        with open(f'imgoutputs-i2i/{curr_block}/prompt.txt', 'w') as f:
            f.write(prompt)
        bt.logging.trace("Finished!")
